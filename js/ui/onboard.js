...
export function openOnboarding({ edit = false } = {}) {
  ...
  const a = {
    ...
    equipment: p.equipment || null, injuries: [...(p.injuries || [])], cardio: p.cardio || null, plan: null
  };
  const list = steps(edit);
  let i = 0, saved = false;

  const profile = () => sanitizeProfile({ name: a.name, birthYear: year() - a.age, sex: a.sex, heightCm: a.height, level: a.level, goal: a.goal, days: a.days, minutes: a.minutes, equipment: a.equipment, injuries: a.injuries, cardio: a.cardio, notes: p.notes || '' });
  const save = async () => {
    if (saved) return;
    saved = true;
    const pr = profile();
  ...
  const api = openSheet(box => {
    const render = (dir = 1) => {
      ...
      box.innerHTML = `<div class="ob">
        ...
        <div class="obstep ${dir > 0 ? 'in-r' : 'in-l'}">${stepHTML(st)}</div>
      </div>`;
    ...
    render(1);
  }, { label: t('ob.title'), onClose: () => { if (!saved) { saved = true; const pr = profile(); store.setSettings({ profile: i > (edit ? 0 : 1) ? pr : state.settings.profile, profileAsked: Date.now(), ...(i > 1 ? derivedSettings(pr) : {}) }); } } });
